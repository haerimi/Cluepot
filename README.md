...
erDiagram
    ...
    Participant {
        ...
        Float lng
        String[] transports
        String distanceTolerance
        String atmospherePreference
        Boolean isHost
        DateTime createdAt
    }
    Schedule {
        String id PK
        String roomCode FK UK
        String title
        String placeName
        String placeAddress
        Float lat
    ...
    User ||--o{ ScheduleMember : "member of"
    User ||--o{ Schedule : "creates"
    Room ||--o{ Participant : "has"
    Room ||--o| Schedule : "has"
    Schedule ||--o{ ScheduleMember : "has"
```
...
**원인:** Server Action에서 `transports: [myTransports]`로 배열 래핑을 누락한 케이스 존재.
...
**문제:** 사용자가 많을 때 Gemini API가 503을 반환하며 추천이 중단됨.
...
### 3. iOS Safari 모바일 그리드 레이아웃

**문제:** `lg:grid`를 모바일에서도 적용했더니 iOS Safari에서 암묵적 트랙이 생성되어 좌측에 ~15px 빈 컬럼이 생김.

**원인:** CSS Grid의 암묵적 트랙 생성 동작.

**해결:** 모바일에서는 `flex flex-col`로 단일 컬럼 레이아웃을 사용하고, `lg:grid`는 데스크탑에서만 적용.

---

### 4. 좌표 획득 실패 시 AI Fallback

**문제:** 카카오 지오코딩 API가 일부 지역명("홍대 근처" 등)을 처리하지 못해 중심점 계산 불가.
