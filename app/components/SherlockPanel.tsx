...
/**
 * SherlockPanel — two rendering modes
 *
 ...
 *
 * Loading state:
 *   Replaces the radar/scanner animation with SherlockTypographicLoader —
...
function SherlockTypographicLoader() {
  ...
  useEffect(() => {
    if (show) return;
    const t = setTimeout(() => {
      setIdx((i) => (i + 1) % SHERLOCK_THOUGHTS.length);
      setShow(true);
    }, 300);
    ...
    <div className="flex flex-col items-center justify-center flex-1 gap-6 px-10 py-20">
      ...
      <div className="w-32 h-px bg-hairline relative overflow-hidden">
        <div
          ...
          style={{ animation: "sherlock-progress 2.2s ease-in-out infinite" }}
        />
      </div>
    </div>
  );
