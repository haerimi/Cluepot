...
/**
 * WatsonPanel — two rendering modes
 *
 ...
 *
 * Loading state:
 *   Replaces the radar/scanner animation with WatsonTypographicLoader —
...
function WatsonTypographicLoader() {
  const [idx, setIdx] = useState(0);
  const [show, setShow] = useState(true);

  /* Two separate effects avoid nesting beyond 4 levels.
     Effect 1: fires setShow(false) on a 2.4 s interval.
     Effect 2: watches `show`; when false, waits 300 ms then advances idx. */
  useEffect(() => {
  ...
  useEffect(() => {
    if (show) return;
    const t = setTimeout(() => {
      setIdx((i) => (i + 1) % WATSON_THOUGHTS.length);
      setShow(true);
    }, 300);
  ...
  return (
    <div className="flex flex-col items-center justify-center flex-1 gap-6 px-10 py-20">
      {/* 💡 max-w-xl 대신 w-full max-w-2xl로 대폭 늘려줍니다 */}
      ...
      <div className="w-32 h-px bg-hairline relative overflow-hidden">
        <div
          ...
          style={{ animation: "watson-progress 2.2s ease-in-out infinite" }}
        />
      </div>
    </div>
  );
...
function AnalysisSummary({ count }: AnalysisSummaryProps) {
  return (
    ...
    >
      ...
      {/* Inline span avoids ambiguous JSX whitespace (S6772) */}
...
function PanelContent({
  ...
  return (
    <>
      {panelState === "loading" && <WatsonTypographicLoader />}
